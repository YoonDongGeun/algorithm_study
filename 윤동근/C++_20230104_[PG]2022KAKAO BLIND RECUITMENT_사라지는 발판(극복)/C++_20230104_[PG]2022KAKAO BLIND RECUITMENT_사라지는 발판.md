...
![](C++_20230104_[PG]2022KAKAO%20BLIND%20RECUITMENT_사라지는%20발판assets/2023-01-04-23-22-53-image.png)

처음에 오후 8시부터 오전 1시까지 했는데 계속 뭔가 잘 안됐다. 머리에 깨달은바는 있는데 계속 헷갈리기도 하고, 구현해도 정답으로 나오지 않았기 때문이다. 하지만 오늘도 다시 시도했는데, 일단 선언한 변수중에 check와 check2를 혼동하여 쓴 부분도 있고, dx[i]를 써야하는데 dx[0]을 써버리기도 한 부분도 있었다. 그리고 이를 고친뒤에 다시 원래 생각했던 코드대로 작성하니 통과하게 돼었다. 처음 코드는 어떻게 통과 되는 것이 많았고, 시간초과도 많았는데 이제는 시간초과도 나지 않았다.
